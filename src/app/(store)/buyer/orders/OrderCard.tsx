...
import { CheckCircle2, Loader2, MapPin, Store, Truck, CreditCard } from 'lucide-react'
import { completeOrderAction, submitReviewAction, simulatePaymentSuccessAction } from './actions'
...
function CompleteOrderButton({ orderId }: { orderId: string }) {
  ...
  return (
    <button
      ...
      disabled={isPending}
      className="bg-[#7C3AED] text-white px-3 py-1.5 text-xs rounded-md hover:bg-[#6D28D9] font-semibold transition-all duration-200 active:scale-95 shadow-sm hover:shadow-md flex items-center gap-1 disabled:opacity-60 disabled:hover:scale-100 disabled:shadow-none"
    >
    ...
    <div className="border border-green-100 bg-green-50 p-3 mt-2 rounded">
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
        ...
        <button
          type="submit"
          disabled={isPending}
          className="bg-white border border-[#7C3AED] text-[#7C3AED] px-3 py-1.5 text-xs rounded-md hover:bg-[#FAF5FF] font-medium transition-all duration-200 active:scale-95 flex items-center justify-center gap-1 disabled:opacity-60 flex-shrink-0"
        >
          {isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : null}
...
export default function OrderCard({ trx }: OrderCardProps) {
  ...
  useEffect(() => {
    // Load Midtrans Snap script if it hasn't been loaded yet
    if (trx.paymentStatus === 'PENDING' && trx.paymentToken) {
  ...
  }, [trx.paymentStatus, trx.paymentToken])
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden border border-gray-100">
      ...
      {/* HEAD TRANSAKSI */}
      <div className="p-3 md:p-4 border-b border-gray-100 bg-gray-50 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-[#7C3AED] font-semibold text-sm">{formatRupiah(Number(trx.totalAmount))}</span>
          ...
          {trx.paymentStatus === 'PENDING' && trx.paymentToken && (
            <div className="flex items-center gap-2">
              <button
                onClick={() => startTransition(() => simulatePaymentSuccessAction(trx.id))}
                disabled={isPending}
                className="bg-gray-200 text-gray-700 px-3 py-1.5 text-xs rounded-md hover:bg-gray-300 font-medium transition-all duration-200 active:scale-95 shadow-sm hover:shadow-md flex items-center disabled:opacity-50"
              >
              ...
              <button
                onClick={() => {
                  if (typeof window !== 'undefined' && (window as any).snap) {
                    (window as any).snap.pay(trx.paymentToken, {
                      onSuccess: function() {
                ...
                className="bg-[#7C3AED] text-white px-3 py-1.5 text-xs rounded-md hover:bg-[#6D28D9] font-medium transition-all duration-200 active:scale-95 shadow-sm hover:shadow-md flex items-center gap-1.5"
              >
          ...
          <div key={order.id} className="p-3 md:p-4 border-b border-dashed border-gray-100 last:border-0">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-3">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button className="bg-gray-100 text-gray-600 px-2 py-1 text-xs rounded-md border border-gray-200 hover:bg-gray-200 transition-all duration-200 active:scale-95">Chat Penjual</button>
              </div>
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  {order.shippingCourier
                    ? <><div>{order.shippingCourier}</div><div>Ongkir: <span className="text-[#7C3AED] font-medium">{formatRupiah(Number(order.shippingCost))}</span></div></>
